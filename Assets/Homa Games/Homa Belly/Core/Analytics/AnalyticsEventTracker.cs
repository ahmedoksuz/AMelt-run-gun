...
namespace HomaGames.HomaBelly
{
    public static class AnalyticsEventTracker
    {
        ...
        public static EventId TrackInAppPurchaseEvent(ProductCategory productCategory, string productId, string currencyCode, double unitPrice, CurrencyType currencyType = CurrencyType.Other, int amount = 0, string transactionId = null, string payload = null, bool isRestored = false)
        {
            EventId eventId = new IAPPurchaseCompleted(productCategory, productId, currencyType, amount, transactionId).TrackEvent();
        ...
        /// <param name="currencyType">(Optional) The type of currency purchased</param>
        /// <param name="amount">(Optional) The amount of currency purchased</param>
        public static EventId TrackInAppPurchaseEvent(UnityEngine.Purchasing.Product product, bool isRestored = false, CurrencyType currencyType = CurrencyType.Other, int amount = 0)
        {
            EventId eventId = new IAPPurchaseCompleted(product.definition.type == ProductType.Subscription ? ProductCategory.Subscription : ProductCategory.Other,
                product.definition.id, currencyType, amount, product.transactionID).TrackEvent();
